...
export class BranchService {
            ...
            data: {
                ...
                manufacturer:
                    manufacturer && manufacturer.length > 0
                        ? {
                            connect: manufacturer.map((id: string) => ({ id })),
                        }
                        : undefined,
                personInCharge:
                    personInCharge && personInCharge.length > 0
                        ? {
                            connect: personInCharge.map((id: string) => ({ id })),
                        }
                        : undefined,
            },
            include: {
    ...
    static async updateBranch(id: string, data: any, userId?: string) {
        const {
            name,
            gst,
            email,
        ...
        const updatedBranch = await prisma.branch.update({
            ...
            data: {
                ...
                manufacturer: manufacturer
                    ? {
                        set: manufacturer.map((id: string) => ({ id })),
                    }
                    : undefined,
                personInCharge: personInCharge
                    ? {
                        set: personInCharge.map((id: string) => ({ id })),
                    }
                    : undefined,
            },
            include: {
