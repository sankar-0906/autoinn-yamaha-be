...
export class HsnController {
    static async createHsn(req: Request, res: Response) {
        try {
    ...
    static async getAllHsns(req: Request, res: Response) {
        try {
    ...
    static async getHsnById(req: Request, res: Response) {
        try {
            const { id } = req.params;
            if (!id) return sendError(res, 'HSN ID is required', 400);

            const hsn = await HsnService.getHsnById(id);
            if (!hsn) return sendError(res, 'HSN not found', 404);
    ...
    static async updateHsn(req: Request, res: Response) {
        try {
            const { id } = req.params;
            if (!id) return sendError(res, 'HSN ID is required', 400);
    ...
    static async deleteHsn(req: Request, res: Response) {
        try {
            const { id } = req.params;
            if (!id) return sendError(res, 'HSN ID is required', 400);
